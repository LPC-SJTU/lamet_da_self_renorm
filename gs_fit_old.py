...
def t_plot(a_str='a06', file_path='DA_new.hdf5'):
    ...
    for z in [3]:
        ...
        ax.errorbar(np.arange(1, Nt+1)[:t_max], [val.mean-0.01 for val in real][:t_max], yerr=[val.sdev for val in real][:t_max], color=color_list[3], label='Data', **errorb)
        ...
        ax.legend(loc='upper right', ncol=2, **fs_p)
