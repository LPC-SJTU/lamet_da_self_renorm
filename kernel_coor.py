...
# %%
lc_re_ls = gv.load('./pion/mom=8/lc_re_ls')
lc_im_ls = gv.load('./pion/mom=8/lc_im_ls')
quasi_re_ls = gv.load('pion/mom=8/a_hyb_re_ls')
quasi_im_ls = gv.load('pion/mom=8/a_hyb_im_ls')
...
ax.fill_between(lam_ls, [(val.mean + val.sdev) for val in re_avg], [(val.mean - val.sdev) for val in re_avg], color='blue', alpha=0.6, label='lc')
...
fig = plt.figure(figsize=fig_size)
ax = plt.axes(plt_axes)
ax.fill_between(lam_ls, [(val.mean + val.sdev) for val in im_avg], [(val.mean - val.sdev) for val in im_avg], color='blue', alpha=0.6, label='lc')
